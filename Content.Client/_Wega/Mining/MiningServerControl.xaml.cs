...
public sealed partial class MiningServerControl : Control
{
    ...
    public void UpdateData(MiningServerData data)
    {
        ...
        // LP edit start
        // состояния платы
        if (data.CircuitboardCondition <= 0)
            CircuitboardStatusLabel.SetMessage(FormattedMessage.FromMarkupPermissive(Loc.GetString("mining-server-circuitboard-examined-broken")));
        else
            CircuitboardStatusLabel.SetMessage(FormattedMessage.FromMarkupPermissive(Loc.GetString("mining-server-circuitboard-examined", ("condition", data.CircuitboardCondition.ToString("F0")))));
        // LP edit end
