...
public sealed class InducerSystem : EntitySystem
{
    ...
    private void OnDoAfter(EntityUid uid, InducerComponent component, DoAfterEvent args)
    {
        ...
        energyToTransfer = Math.Min(energyToTransfer, sourceBattery.LastCharge);

        var freeSpace = targetBattery.MaxCharge - targetBattery.LastCharge;
        ...
        if (_battery.TryUseCharge((slot.Item.Value, sourceBattery), energyToTransfer))
        {
            ...
            var percent = (int)(targetBattery.LastCharge / targetBattery.MaxCharge * 100);
            _popup.PopupEntity(Loc.GetString("inducer-success", ("percent", percent)), uid, args.User);

            if (targetBattery.LastCharge < targetBattery.MaxCharge * 0.95f)
            {
                args.Repeat = true;
            }
            else
        ...
        {
            _battery.SetCharge((target, targetBattery), targetBattery.LastCharge + energyToTransfer);
            _battery.SetCharge((slot.Item.Value, sourceBattery), sourceBattery.LastCharge - energyToTransfer);
            args.Repeat = false;
        }
    }
